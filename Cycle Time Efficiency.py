...
def load_base_data():
    ...
    shared_parts = ['Part-001', 'Part-002', 'Part-003', 'Part-004', 'Part-005']
    ...
    parts_f = np.random.choice(shared_parts + ['Part-006', 'Part-007'], N_FAST)
    ...
    parts_s = np.random.choice(shared_parts + ['Part-008', 'Part-009'], N_SLOW)
    ...
    parts_w = np.random.choice(shared_parts + [f"Part-{i:03d}" for i in range(10, 25)], N_WITHIN)
    ...
    df_fast = pd.DataFrame({
        ...
        'Product': products_f,
        'Part': parts_f,
        'Tooling': np.random.choice([f"TL-{i:03d}" for i in range(1, 15)], N_FAST)
    })
    ...
    df_slow = pd.DataFrame({
        ...
        'Product': products_s,
        'Part': parts_s,
        'Tooling': np.random.choice([f"TL-{i:03d}" for i in range(15, 25)], N_SLOW)
    })
    ...
    df_within = pd.DataFrame({
        ...
        'Part': parts_w,
        'Tooling': np.random.choice([f"TL-{i:03d}" for i in range(25, 45)], N_WITHIN)
    })
